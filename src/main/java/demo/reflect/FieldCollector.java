 ...
 *
 * 1.批量的
 *      1).Field[] getFields():获取所有的"公有字段" 包含继承的
 *      2).Field[] getDeclaredFields():获取所有字段，包括：私有、受保护、默认、公有；不包含继承的
 * 2.获取单个的：
 *      1).public Field getField(String fieldName):获取某个"公有的"字段； 包含继承的
 *      2).public Field getDeclaredField(String fieldName):获取某个字段(可以是私有的) 不包含继承的
 *
 *   设置字段的值：
